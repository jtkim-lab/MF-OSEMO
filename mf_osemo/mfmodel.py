...
class MFGPRegressor:
    ...
    def predict(self, x, cov=True, var=False):
        ...
        if cov == True:
            ...
            for m in range(0, self.M-1):
                cov = self.kernel.diag(self.x_test[m*self.size:(m+1)*self.size]) - np.einsum(
                    'ij,ji->i', temp[(self.M-1)*self.size:, :], K.T[:, m*self.size:(m+1)*self.size])
                cov_k = np.r_[cov_k, cov]
            return self.mean * self.y_std + self.y_mean, self.std * self.y_std, cov_k * np.power(self.y_std,2)
        else:
    ...
    def optimized_predict(self, x, cov=True):
        ...
        if cov == True:
            ...
            for m in range(0, self.M-1):
                cov = self.kernel.diag(self.x_test[m*self.size:(m+1)*self.size, :]) - np.einsum(
                    'ij,ji->i', v.T[(self.M-1)*self.size:, :], v[:, m*self.size:(m+1)*self.size])
                cov_k = np.r_[cov_k, cov]
            return self.mean * self.y_std + self.y_mean, self.std * self.y_std, cov_k * np.power(self.y_std,2)
        else:
            return self.mean * self.y_std + self.y_mean, self.std * self.y_std
...
class MFGPKernel:
    ...
    def set_params(self, sigma_f, error_sigma, ell):
        self.kernel_f.set_params(
            k1__constant_value=sigma_f, k2__length_scale=ell)
        self.kernel_e.set_params(k1__constant_value=error_sigma, k2__length_scale=ell)

    def calc_diff(self, x1, x2):
        error_constant = np.concatenate([np.c_[x1[:, 0]].dot(np.c_[np.ones(np.size(x2, 0))].T), np.c_[np.ones(np.size(x1, 0))].dot(np.c_[(x2[:, 0])].T)], axis=0).reshape(2, np.size(x1, 0), np.size(x2, 0))
